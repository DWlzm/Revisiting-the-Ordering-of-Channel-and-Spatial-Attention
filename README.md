...
This paper can be accessed for free at https://arxiv.org/pdf/2601.07310. If you have any questions, please contact 2033771388@qq.com .

🎉 "Paper accepted at Signal, Image and Video Processing! 🚀🔥 https://link.springer.com/article/10.1007/s11760-026-05312-7
## Abstract
...
class  ChannelandSpatialAdaptiveFusionAttention (nn.Module):
    def __init__(self, in_channels: int, reduction: int = 16, kernel_size: int = 7):
        super().__init__()
        self.ca = ChannelAttention(in_channels, reduction)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return alpha * self.ca(x) + (1 - alpha) * self.sa(x)


def main():
    torch.manual_seed(42)
    x = torch.randn(1, 64, 28, 28)
    model = ChannelandSpatialAdaptiveFusionAttention(64, 16, 7)
    model.eval()
    with torch.no_grad():
        out = model(x)
    print(f"in: {x.shape}  ->  out: {out.shape}")
...
class BidirectionalChannelSpatialAttention(nn.Module):
    def __init__(self, in_channels: int, reduction: int = 16):
        super().__init__()
        self.ca = ChannelAttention(in_channels, reduction)
        self.sa = SpatialAttention()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.sa(self.ca(x)) + self.ca(self.sa(x))


def main():
    torch.manual_seed(42)
    x = torch.randn(2, 64, 32, 32)
    model = BidirectionalChannelSpatialAttention(64, 16)
    model.eval()
    with torch.no_grad():
        out = model(x)
    print(f"in: {x.shape}  ->  out: {out.shape}")
...
class GatedChannelSpatialAdditiveAttention(nn.Module):
    def __init__(self, in_channels: int, reduction: int = 16):
        super().__init__()
        self.ca = ChannelAttention(in_channels, reduction)
        self.sa = SpatialAttention()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        a_s = self.sa(x)
        g_c = self.gate_c(x)
        g_s = F.adaptive_avg_pool2d(self.gate_s(x), 1)
        w = torch.softmax(torch.cat([g_c, g_s], dim=1).view(x.size(0), 2), dim=1)
        w_c, w_s = w[:, 0:1, None, None], w[:, 1:2, None, None]
        return w_c * a_c + w_s * a_s


def main():
    torch.manual_seed(42)
    x = torch.randn(2, 64, 32, 32)
    model = GatedChannelSpatialAdditiveAttention(64, 16)
    model.eval()
    with torch.no_grad():
        out = model(x)
    print(f"in: {x.shape}  ->  out: {out.shape}")
...
class TGPFA(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ca_out = self.ca(x)
        sa_out = self.sa(x)
        w      = torch.softmax(self.gate(x), dim=1)          # [B,3,1,1]
        return (w[:, 0:1] * orig +
                w[:, 1:2] * ca_out +
                w[:, 2:3] * sa_out)
